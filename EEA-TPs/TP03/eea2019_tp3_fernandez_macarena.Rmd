...
- **modelo 1:** predice la probabilidad de sobrevivir en función del monto pagado por el pasaje y el sexo
...
- **Modelo 2:** solo las variables *Sex* y *Pclass* resultan significativas. Los coeficientes asociados a *Embarked* presentan un valor mayor a 0,05, por lo que no aportan información al modelo. En el caso de *Sex* y *Pclass* se observa la misma influencia en la probabilidad que la encontrada en el modelo 0.

- **Modelo 3:** aquí la variable *Sex* aporta infromación pero la variable *Embarked* no. En el caso de *Sex*, tiene la misma influencia sobre la probabilidad predicha que la que tenía en el modelo 0. *Embarked*, por su parte, influye positivamente cuando la clase de embarcación es la C e influye negativamente cuando es la Q.

A continuación, se ordenan los modelos por deviance explicada. Podemos ver que el modelo que minimiza la deviance es el modelo 2, lo que hace que su porcentaje de deviance explicada sea mayor al resto: aproximadamente 31%.
...
En este apartado procedemos a evaluar el mejor modelo ajustado en el apartado anterior, esto es, el modelo 2. En primer lugar, realizamos una curva ROC.
