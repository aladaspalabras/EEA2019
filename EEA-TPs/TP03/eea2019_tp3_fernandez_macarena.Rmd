...
- **modelo 1:** predice la probabilidad de sobrevivir en función de la tarifa pagada por el pasaje y el sex
- **modelo 2:** predice la probabilidad de sobrevivir en función de la clase, el sexo, la edad y la tarifa pagada por el pasaje
- **modelo 3:** predice la probabilidad de sobrevivir en función del sexo y la cantidad de pares y/o hijos abordo del barco
...
La *sensitivity*, a la cual también se la suele denominar *recall*, indica la cuántos casos realmente positivos se lograron predecir correctamente. Para ello, se considera el total de casos que el modelo predijo como positivos y que realmente lo son (TP) sobre el total de positivos relaes (TP+FN), hayan sido predecidos como tales por el modelo (TP) o hayan sido catalogados errónemanete como negativos (FN). En este trabajo, los verdaderos son los sobrevivientes que intentamos predecir. 

La *specificity*, por otro lado, indica lo mismo con respecto a los negativos: cuántos casos que en realidad eran negativos fueron predichos de esta forma por nuestro modelo. Aquí la cuenta que se realiza toma los negativos reales que fueron predichos de tal forma por nuestro modelo (TN) y los divide por el total de negativos reales (TN+FP). En este trabajo, los casos negtivos corresponden a los "no sobrevivientes".

Además, podemos calcular el área bajo la curva graficada. El modelo 2 cuenta con un área del 83%, por lo que sus predicciones superan el las del azar, cuyo área es del 50% y se encuentra representado por la línea punteada.
...
En él se puede observar que, a medida que la probabilidad asignada por el modelo aumenta, la clase "no sobreviviente" disminuye su densidad y la clase "sobreviviente" la aumenta. Esto nos ayuda a pensar un posible punto de corte a partir del cual podemos establecer que una persona sobrevive, intentando además no predecir erróneamente los "no sobrevivientes". Dado que el quiebre entre disminución de la densidad de "no sobrevivientes" y aumento de la densidad de "sobrevivientes" parece estar entre 0.30 y 0.45, podría pensarse un punto de corte que esté en ese intervalo.
...
El *accuracy* mide la performance general del modelo calculando cuándos casos predijo correctamente, sean estos positivos (aquí, "sobrevivientes") o negativos ("no sobrevivientes"), y nos indica qué proporción representan los mismos respecto del total de casos predichos (TP+FP+TN+FN).

La *precision*, a su vez, 
