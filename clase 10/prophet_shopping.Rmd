...
La funcion `cross_validation` permite realizar pronosticos realizando un esquema de cross-validation temporal y, a partir de ellos, obtener ciertas metricas de performance.
  ...
  * **horizon**: horizonte del pronóstico. Cuánto tiempo deseo predecir en el futuro.
  
  * **period**: período entre fechas de análisis. Cuánto voy corriendo mi ventana.
  
   * **initial**: período inicial de entrenamiento (no testeo con esto, lo uso para entrenar no más).
...
La función `performance_metrics` computa varias metricas de performance a partir de un dataframe de cross validation de prophet
